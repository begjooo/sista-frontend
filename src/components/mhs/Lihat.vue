<script setup>
import { ref } from 'vue';
import { baseUrl } from '@/baseUrl';
import {
  Dialog,
  DialogClose,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from '@/components/ui/dialog'

const props = defineProps(['mhsUsername'])

const mhsDataPribadi = ref();
const mhsCv = ref();

async function lihatMhs() {
  console.log(`lihatMhs`)
  try {
    const response = await fetch(`${baseUrl}/mhs/${props.mhsUsername}/data`)
    const data = await response.json()

    if (!data) {
      mhsDataPribadi.value = null
      mhsCv.value = null
    } else {
      mhsDataPribadi.value = data.pribadi
      mhsCv.value = data.cv
    }
  } catch (error) {
    console.log(error.message)
  }
}
</script>

<template>
  <div>
    <Dialog>
      <DialogTrigger as-child>
        <svg class="text-orange-600 hover:text-orange-700 cursor-pointer" @click="lihatMhs"
          xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
          <path fill="currentColor"
            d="M11 12q-1.65 0-2.825-1.175T7 8t1.175-2.825T11 4t2.825 1.175T15 8t-1.175 2.825T11 12m11.1 11.5l-3.2-3.2q-.525.3-1.125.5T16.5 21q-1.875 0-3.187-1.312T12 16.5t1.313-3.187T16.5 12t3.188 1.313T21 16.5q0 .675-.2 1.275t-.5 1.125l3.2 3.2zM16.5 19q1.05 0 1.775-.725T19 16.5t-.725-1.775T16.5 14t-1.775.725T14 16.5t.725 1.775T16.5 19m-5.475-6q-1.05 1.55-1.05 3.5t1.05 3.5H3v-2.775q0-.85.425-1.575t1.175-1.1q1.275-.65 2.875-1.1t3.55-.45" />
        </svg>
      </DialogTrigger>
      <DialogContent class="min-w-[300px]">
        <DialogHeader>
          <DialogTitle>Data Mahasiswa</DialogTitle>
          <DialogDescription></DialogDescription>
        </DialogHeader>

        <div v-if="mhsDataPribadi">
          <div class="text-sm max-h-[400px] overflow-y-auto px-2 py-1">
            <div class="">
              <div class="flex flex-col font-semibold text-center">
                <div>{{ mhsDataPribadi.name }} [{{ mhsDataPribadi.username }}]</div>
                <div>{{ mhsDataPribadi.kelas }} | {{ mhsDataPribadi.prodi }} | {{
                  mhsDataPribadi.tahun_ajaran }}</div>
              </div>

              <div class="mt-2">
                <div class="font-semibold">Curriculum Vitae</div>
                <div v-if="mhsCv && mhsCv.length !== 0">
                  <div v-for="cv in mhsCv">
                    <div>{{ cv }}</div>
                  </div>
                </div>
                <div v-else>
                  Tidak ada data
                </div>
              </div>
            </div>
          </div>
        </div>

        <DialogFooter></DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
</template>