...
<template>
  <div class="text-sm">
    <div class="flex flex-col text-center my-2 border-y py-2">
      <div class="text-base font-semibold mb-1 text-blue-800">Kelompok Bidang Keahlian</div>
    ...
    <div v-if="selectedKbk" class="flex flex-wrap gap-2 px-2 min-h-[60vh] max-h-[60vh] overflow-auto">
      <div class="border rounded-sm w-[300px]">
      ...
      <div v-if="selectedDosen" class="border rounded-sm flex-1">
        <div class="text-center pt-2 pb-1 font-semibold text-orange-600">Minat Penelitian</div>
        <div v-if="selectedDosen" class="col-span-3 border-b mb-2">
          <div class="flex flex-wrap gap-2 justify-center mb-2">
            <div v-for="minat in selectedDosen.minat" class="cursor-pointer">
              <div class="border text-center rounded-sm px-3 py-1 hover:bg-gray-100 hover:font-semibold"
        ...
        <div class="flex flex-col gap-2">
          <div v-if="judulPerMinat && judulPerMinat.length === 0" class="text-center px-2 pt-1">
            Dosen terkait ({{ selectedDosen.fullname }}) <span class="font-semibold">tidak mengusulkan</span>
            judul pada bidang peminatan {{ selectedMinat }}
          </div>
          <div v-else>
              ...
              <div class="flex space-x-2 p-2">
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <div v-if="selectedJudul.type === 'mahasiswa'">
                    <div class="">
                      <div class="flex flex-wrap">
                        <div class="w-[100px] text-gray-600">Pbb Utama</div>
                        <div>{{ selectedDosen.fullname }}</div>
                      </div>
                      <div class="flex flex-wrap">
                        <div class="w-[100px] text-gray-600">KBK</div>
                        <div>{{ selectedKbk }}</div>
                      </div>
                      <div class="flex flex-wrap">
                        <div class="w-[100px] text-gray-600">Minat</div>
                        <div>{{ selectedMinat }}</div>
                      </div>
                    </div>
                    <div class="flex flex-wrap border-t border-blue-200 mt-1 pt-1">
                      <label for="skema" class="content-center text-gray-600 w-[100px]">Skema</label>
                    ...
                    <div>
                      <div class="text-gray-600">Judul/Topik</div>
                      <input
                        class="border-b border-orange-600 resize-none focus:outline-0 focus:border-blue-800 w-[50vh] pb-2"
                        placeholder="Judul/Topik Tugas Akhir ..." v-model="selectedJudul.judul" />
                    </div>
                    <div>
                      <div class="text-gray-600 mt-2">Deskripsi</div>
                      <textarea rows="4"
                        placeholder="Deskripsi Tugas Akhir seperti Abstrak yang mencakup latar belakang, tujuan, dan sebagainya, serta Referensi Utama"
          ...
          <div v-if="selectedMinat">
            <Dialog>
              ...
              <DialogContent class="min-w-[200px] text-sm">
                <DialogHeader>
                  <DialogTitle>Rekapitulasi Usulan Pembimbing</DialogTitle>
