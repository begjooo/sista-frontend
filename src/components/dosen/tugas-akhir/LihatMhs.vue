...
const props = defineProps(['mhs', 'taId'])
...
async function tolak(taId, mhsUsername) {
  console.log(`tolak ${taId} ${mhsUsername}`)
  try {
...
<template>
  <Dialog>
    <DialogTrigger as-child>
      <span class="font-semibold cursor-pointer hover:underline" @click="lihatMhs(props.mhs.username)">
        {{ props.mhs.name }}
      </span>
    </DialogTrigger>
    <DialogContent class="min-w-[300px]">
      ...
      <div v-else class="text-center text-red-600 font-semibold">
        Data {{ props.mhs.name }} NIM {{ props.mhs.username }} tidak ditemukan!
      </div>
      ...
      <DialogFooter>
            ...
            <DialogContent class="min-w-[200px]">
              ...
              <DialogFooter>
                ...
                <Button class="cursor-pointer w-[100px]"
                  @click="diskusiUsulan(props.taId, props.mhs.username, props.mhs.name, props.mhs.degree)">Send</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </DialogClose>
        <DialogClose as-child>
          <Button variant="destructive" class="cursor-pointer w-[100px]"
            @click="tolak(props.taId, props.mhs.username)">Tolak</Button>
        </DialogClose>
      </DialogFooter>
    </DialogContent>
  </Dialog>
